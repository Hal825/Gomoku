...
public:
    ...
    bool select_by_name(const std::string &name, Json::Value &user)
    {
        char sql[4096] = {0};
        snprintf(sql, sizeof(sql),
                 "SELECT id, score, total_count, win_count FROM user WHERE username='%s';", name.c_str());
    ...
    bool select_by_id(uint64_t id, Json::Value& user)
    {
        char sql[4096] = {0};
        snprintf(sql, sizeof(sql),
                 "SELECT username, score, total_count, win_count FROM user WHERE id=%lu;",
                 id);
    ...
    bool win(uint64_t id) {
        char sql[4096] = {0};
        snprintf(sql, sizeof(sql),
                 "UPDATE user SET score=score+30, total_count=total_count+1, win_count=win_count+1 WHERE id=%lu;",
                 id);
    ...
    bool lose(uint64_t id) {
        char sql[4096] = {0};
        snprintf(sql, sizeof(sql),
                 "UPDATE user SET score=score-30, total_count=total_count+1 WHERE id=%lu;",
                 id);
